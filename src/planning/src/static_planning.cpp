...
class StaticNodelet : public nodelet::Nodelet {
 private:
  ...
  void debug_timer_callback(const ros::TimerEvent& event) {
    ...
    iniState.setZero();
    iniState.col(0) = uav_p_;
    iniState.col(1) = uav_v_;
    iniState.col(2) = uav_a_;
    iniState.col(3) = uav_j_;
    ...
    for (double t = 0; t <= traj.getTotalDuration(); t += dt) {
      // std::cout << "visualize odom time:" << t << std::endl;
      ...
      if (debug_replan_ && t > 1.0 / plan_hz_ && traj.getTotalDuration() > 0.5) {
        ...
        std::cout << "iniState: \n"
                  << iniState << std::endl;
        trajOptPtr_->generate_traj(iniState, target_p, target_v, land_q, uav_q_, false, 10, traj, &plan_state, t);
  ...
  void init(ros::NodeHandle& nh) {
    ...
    nh.getParam("uav_p_x", uav_p_.x());
    nh.getParam("uav_p_y", uav_p_.y());
    nh.getParam("uav_p_z", uav_p_.z());
    nh.getParam("uav_v_x", uav_v_.x());
    nh.getParam("uav_v_y", uav_v_.y());
    nh.getParam("uav_v_z", uav_v_.z());
    nh.getParam("uav_a_x", uav_a_.x());
    nh.getParam("uav_a_y", uav_a_.y());
    nh.getParam("uav_a_z", uav_a_.z());
    nh.getParam("uav_j_x", uav_j_.x());
    nh.getParam("uav_j_y", uav_j_.y());
    nh.getParam("uav_j_z", uav_j_.z());
    nh.getParam("uav_q_x", uav_q_.x());
    nh.getParam("uav_q_y", uav_q_.y());
    nh.getParam("uav_q_z", uav_q_.z());
    nh.getParam("uav_q_w", uav_q_.w());
    ...
    triger_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("triger", 10, &StaticNodelet::triger_callback, this, ros::TransportHints().tcpNoDelay());
 ...
 public:
  void onInit(void) {
    ...
    initThread_ = std::thread(std::bind(&StaticNodelet::init, this, nh));
  }
...
PLUGINLIB_EXPORT_CLASS(planning::StaticNodelet, nodelet::Nodelet);
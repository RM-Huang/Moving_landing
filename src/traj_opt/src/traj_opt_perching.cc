...
static double traking_height = 3.0;
                            ...
                            const double& t_replan) 
{
  ...
  {
    traj_tail_alt = car_p_[2] + traking_height;
  ...
  Trajectory traj_tmp = mincoOpt_.getTraj(); //调用MINCO轨迹类生成轨迹
  ...
  std::cout << "maxVel_Z:" << getMaxVelZ(traj_tmp) << std::endl;
...
TrajOpt::TrajOpt(ros::NodeHandle& nh) {
  ...
  nh.getParam("visual_region", visual_region_);
...
double TrajOpt::check_visible(const Eigen::Vector3d& pos,
                            const Eigen::Vector3d& acc,
                            const Eigen::Vector3d& car_p){
  Eigen::Vector3d pc = - car_p + pos;
  double dist_sqr = pc.squaredNorm();
